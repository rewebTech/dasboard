...
// ── Token Management ─────────────────────────────────────
const TOKEN_KEY = process.env.NEXT_PUBLIC_TOKEN_KEY || 'sh_token';
...
/** Save full user session data (user, dashboard, subscription) */
export function setUserSession(data) {
  setStorage('sh_user', data.user || null);
  setStorage('sh_dashboard', data.dashboard || null);
  setStorage('sh_subscription', data.subscription || null);
}

/** Get stored user session */
export function getUserSession() {
  return {
    user: getStorage('sh_user'),
    dashboard: getStorage('sh_dashboard'),
    subscription: getStorage('sh_subscription'),
  };
}
...
export function clearUserSession() {
  ...
  localStorage.removeItem('sh_dashboard');
  localStorage.removeItem('sh_subscription');
}
...
/** Format as INR currency: 2999 → "₹2,999" */
